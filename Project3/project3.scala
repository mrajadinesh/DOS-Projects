...
case class StartSendingRequests(nodeMap: TreeMap[String,ActorRef], distanceNodes: HashMap[Int,String], numNodes: Int, numRequests: Int)
...
case class RouteRequest(id: String,masterActor: ActorRef,msg: String,count: Int)
...
class PastryActor extends Actor{
  ...
  case RouteRequest(id,masterActor,msg,numHops)=>
    ...
    { 
      ...
      else
      {
        ...
        if((leafSet(0) != null) && (Integer.parseInt(leafSet(0)) <= Integer.parseInt(id) && Integer.parseInt(leafSet(leafSetSize-1)) >= Integer.parseInt(id)))
        {
          ...
          {
            if((leafSet(i)!=null) && (math.abs(Integer.parseInt(leafSet(i))-Integer.parseInt(id)) < distance))
            {
        ...
        if(flag == 1)
          nodeMap.apply(nextNodeId) ! RouteRequest(id,masterActor,msg,numHops+1)
      }
    }
    catch 
    {  
...
class MasterActor extends Actor{
  ...
  def receive = {
    ...
    case StartSendingRequests(nodeMap,distanceNodes,numNodes,numRequests)=>
      ...
      for(i<-0 until numNodes by 1)
      {
        ...
        while(reqCount > 0)
        {
		  ...
		  nodeMap.apply(sourceNode)! RouteRequest(destinationNode,self,"PastryMessage",1)
		  reqCount=reqCount-1
...
object project3
{
  ...
  def main(args: Array[String]): Unit = 
  {
    ...
    BuildNetwork(numNodes, numRequests)
  }
  
  //Build Network
  def BuildNetwork(numNodes: Int, numRequests: Int)
  {
    var j: Int=0
    var k: Int=0
    var n: Int=numNodes
    ...
    mActor ! StartSendingRequests(nodeMap,distanceNodes,numNodes,numRequests)
  }
}
